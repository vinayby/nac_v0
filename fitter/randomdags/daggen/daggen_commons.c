...
int parseOptions(int argc, char *const *argv) {
  ...
  while (ret_val == 0) {
    ...
    case 0:
      ...
      }  else if (!strcmp(optname, "rseed")) {
        int rseed = atoi(optarg);
        rseed_flag = 1;
        global.rseed = rseed;
      }
  ...
  if( ret_val == 0 ) {
    ...
    if (!rseed_flag) {
      fprintf(stderr,"Warning: seeding srand based on getpid + time()\n");
    }
    if (!density_flag) {
...
void printUsage(void)
{
  ...
  fprintf(stderr,
      ...
      "\t --rseed <specify-a-seed-to-use (default and passing 0 uses getpid() + time())>\n"
