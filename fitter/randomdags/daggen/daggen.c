...
int main(int argc, char **argv)
{
  ...
  if (global.rseed == 0){
    global.rseed = (unsigned int)getpid() + (unsigned int)time(NULL);
  }
  srand(global.rseed);
  fprintf(stderr, "RSEED %u\n", global.rseed);
  ...
  for (i=0; i<dag->nb_levels-1; i++) {
    for (j=0; j<dag->nb_tasks_per_level[i]; j++) {
      for (k=0; k<dag->levels[i][j]->nb_children; k++) {
        // dag->levels[i][j]->comm_costs[k] = (pow( dag->levels[i][j]->data_size, 2.0));
        dag->levels[i][j]->comm_costs[k] = ((int) getRandomNumberBetween(global.mindata, global.maxdata));
      }
    }
  }
...
//       dag->levels[i][j]->data_size = ((int) getRandomNumberBetween(global.mindata, global.maxdata) / 1024) * 1024;
      dag->levels[i][j]->data_size = ((int) getRandomNumberBetween(global.mindata, global.maxdata));

      op = getRandomNumberBetween(64.0/8, 512.0/8);

//       if (!global.ccr) {
//         dag->levels[i][j]->complexity = ((int) getRandomNumberBetween( global.mindata, global.maxdata) % 3 + 1);
//       } else {
        getRandomNumberBetween(global.mindata, global.maxdata);
        dag->levels[i][j]->complexity = (int)global.ccr;
//       }

      switch (dag->levels[i][j]->complexity) {
      case SIMILAR:
        dag->levels[i][j]->cost = (dag->levels[i][j]->data_size);
        break;
      case N_2:
        dag->levels[i][j]->cost = (    op * pow( dag->levels[i][j]->data_size, 2.0));
        break;
      case N_LOG_N:
        dag->levels[i][j]->cost = (2 * op * pow( dag->levels[i][j]->data_size, 2.0) * (log(dag->levels[i][j]->data_size)/log(2.0)));
        break;
      case N_3:
        dag->levels[i][j]->cost = pow(dag->levels[i][j]->data_size, 3.0);
        break;
      case MIXED:
      ...
      dag->levels[i][j]->alpha = getRandomNumberBetween(global.minalpha, global.maxalpha);
    }
  }
}
